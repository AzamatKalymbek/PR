...
public class AVO {
    ...
    private static Integer gammaParam = 0;

    private static List<HashMap<String, Object>> clusters = new ArrayList<>();

    private static List<Double> epsilonList = new ArrayList<>();

    public static void main(String[] args) {
        start(null, 0);
    }

    public static List<HashMap<String, Object>> start(List<Data> zeroList, Integer classCount) {
        zeroLists = zeroList;
...
//        System.out.println("QUALITY FUNCTIONAL: " + QF);
    ...
    private static boolean checkForAVO(Data obj1, Data obj2) {
        // e = 2 | (m / (m - 1))
        // m = число объектов
        // n - число признаков
    ...
    // epsList
    private static void calcEpsilon() {
        // object count
        int m = dataList.size();
    ...
    private static double calcReverse(int i, int j) {
        return dataList.get(j).getAttributes().get(i);
    }

    private static double calcReverse(int m, int i, int j) {
